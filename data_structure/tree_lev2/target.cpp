...
bool targetsum(TreeNode*root,int sum){
	if(root->left==NULL&&root->right==NULL){
	if(sum==0)return true;
	else return false;
	}
	bool ans1=targetsum(root->left,sum-root->val);
	bool ans2=targetsum(root->right,sum-root->val);
	return ans1||ans2;
...
int main()
{
	...
	int tar;
	cin>>tar;
	puts((targetsum(root,tar))?"true":"false");
	delete root;
    return 0;
}
