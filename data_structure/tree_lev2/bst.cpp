...
int idx=0;
BinaryTreeNode<int>* buildtree(vector<int>&arr,int n,int ninf=INT_MIN,int pinf=INT_MAX){
	if(idx==n||arr[idx]<ninf||arr[idx]>pinf){
		return NULL;
	}
	BinaryTreeNode<int>*root=new BinaryTreeNode<int>(arr[idx++]);
	root->left=buildtree(arr,n,ninf,root->data);
	root->right=buildtree(arr,n,root->data,pinf);
	return root;
}
...
int main()
{
    ...
    BinaryTreeNode<int>*root=buildtree(arr,n);
    display(root);
