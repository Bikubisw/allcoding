...
void pathsumhelper(BinaryTreeNode<int>*root,int tar,vector<vector<int>>&allpath,vector<int>&path){
	if(root==NULL)return;
	...
	pathsumhelper(root->left,tar-root->data,allpath,path);
	pathsumhelper(root->right,tar-root->data,allpath,path);
	path.erase(path.end()-1);
}
vector<vector<int>>pathsum(BinaryTreeNode<int>*root,int tar){
	vector<vector<int>>paths_all;
	vector<int>path;
	pathsumhelper(root,tar,paths_all,path);
	return paths_all;
}
int main()
{
