...
int idx=0;
BinaryTreeNode<int>* buildtree(vector<int>&arr,int start=INT_MAX,int end=INT_MIN){
	if(idx>=arr.size()||arr[idx]<start||arr[idx]>end)return NULL;
...
int main()
{
    ...
    BinaryTreeNode<int>*root=buildtree(arr);
    display(root);
    delete root;
    return 0;
}
